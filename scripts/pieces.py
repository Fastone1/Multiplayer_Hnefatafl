...
class Piece:
    ...
    def move(self, row: int, col: int):
        ...
        self.game.board.set_piece(row, col, self)

        for square in self.game.board.adjacent_squares(row, col):
            if self.check_capture(square[0], square[1]):
                self.game.board.set_piece(square[0], square[1], None)
    ...
    def check_capture(self, row: int, col: int) -> bool:
        ...
        if piece.color != self.color:
            if piece.type == ROOK:
                ...
                return (opposite_piece is not None and opposite_piece.color == self.color) or self.game.board.is_castle(row + direction[0], col + direction[1])
            elif piece.type == KING:
                orthogonal = [(0, 1), (0, -1), (1, 0), (-1, 0)]
                orthogonal_pieces = [self.game.board.get_piece(row + dr, col + dc) for dr, dc in orthogonal]
                return all((piece is not None and piece.color == self.color) or self.game.board.is_castle(row + dr, col + dc) for piece, (dr, dc) in zip(orthogonal_pieces, orthogonal))
